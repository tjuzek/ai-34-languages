...
"""
Generate Gemini continuations for split-halves prompts in 35 languages.
...
- Calls the Gemini API (gemini-1.5-flash-001 by default) with a fixed system prompt.
...
Length control (in model tokens):
  ...
  max_raw        = n * r * 1.5
  max_ceiling    = 200 * r
  max_output_tok = ceil(min(max_raw, max_ceiling))
...
and retrying (up to --max-attempts) when usage.candidates_token_count < min_required_tokens.
...
logger = logging.getLogger("aiwl.generate_gemini")
...
def ensure_trailing_newline(path: Path) -> None:
    """
    Ensure output JSONL file ends with a newline.
    """
    if not path.exists():
        return
    try:
...
def create_client() -> Any:
    ...
    try:
        ...
        raise RuntimeError(
            "Failed to create Gemini client. Make sure GEMINI_API_KEY or "
...
def call_gemini_with_retries(
    ...
    """
    Call the Gemini API with retries and approximate length control
    in terms of model tokens.

    Returns the continuation text, usage dict, request payload, attempts, and whether
    the min_required_tokens target was met (based on usage.candidates_token_count).
    ...
    for attempt in range(1, gen_args.max_attempts + 1):
        ...
        try:
            ...
            logger.debug(
                "Calling Gemini (attempt %d/%d, max_output_tokens=%d, min_required_tokens=%d)",
                attempt,
            ...
            response = client.models.generate_content(**base_payload)

            # Extract continuation text; response.text combines the text parts.
            try:
                continuation_text = (response.text or "").strip()
            except Exception as e:
                raise RuntimeError(f"Unexpected Gemini response structure: {response}") from e
            ...
            if usage_raw is not None:
                if hasattr(usage_raw, "model_dump"):
                    ...
                    usage = {
                        k: v
                        for k, v in getattr(usage_raw, "__dict__", {}).items()
            ...
            # - total_token_count
            out_tokens = int(usage.get("candidates_token_count") or 0)

            # SPECIAL CASE: empty string => accept immediately
            if continuation_text == "":
                logger.info(
                    "Gemini returned empty string; accepting as final output and skipping retries."
                )
                break
    ...
    # JSON-serialisable representation of the request config
    request_payload = {
        "model": gen_args.model,
        "contents": user_prompt,
        "config": {
...
def generate_single_completion(
    ...
    # Map Gemini usage fields into input/output tokens
    input_tokens = int(result.usage.get("prompt_token_count") or 0)
    output_tokens = int(result.usage.get("candidates_token_count") or 0)
...
def process_language_file(
    ...
    Uses a thread pool to run multiple Gemini requests concurrently.
    """
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate Gemini continuations for split-halves prompts in 35 languages."
    )
    p.add_argument(
        "--input-dir",
        type=Path,
    ...
    p.add_argument(
        ...
        help="Gemini model name to use (default: gemini-1.5-flash-001).",
    )
    p.add_argument(
        "--temperature",
        type=float,
    ...
    p.add_argument(
        ...
        default=1,
        help=(
            "Number of concurrent Gemini requests per language (default: 1). "
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    logger.info(
        "Starting Gemini continuation generation run_id=%s for langs=%s with concurrency=%d, rpm=%d",
        run_id,
        ",".join(langs_to_run),
        args.concurrency,
        args.rpm,
