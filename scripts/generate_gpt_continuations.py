...
"""
Generate GPT-4.1 continuations for split-halves prompts in 35 languages.
...
- Calls the OpenAI Responses API (GPT-4.1 by default) with a fixed system prompt.
...
Length control (in model tokens):
  ...
  min_required_tokens_raw = n * r * 0.8
  min_required_floor      = 40 * r
  min_required_tokens     = ceil(min(min_required_tokens_raw, min_required_floor))

  max_output_tokens_raw   = n * r * 1.5
  max_output_ceiling      = 200 * r
  max_output_tokens       = ceil(min(max_output_tokens_raw, max_output_ceiling))

If max_output_tokens < min_required_tokens, set max_output_tokens = min_required_tokens.

The script enforces this approximately by passing max_output_tokens to the API
...
python scripts/generate_gpt_continuations.py \
  --input-dir data/pre-gen-halves-12 \
...
try:
    # New-style OpenAI client (2024+)
    from openai import OpenAI  # type: ignore
except ImportError:  # pragma: no cover - allow script to be imported without openai installed
...
logger = logging.getLogger("aiwl.generate_gpt")
...
def create_client() -> Any:
    if OpenAI is None:
        raise RuntimeError(
            "openai package not installed. Install `openai` >= 1.0 and set OPENAI_API_KEY."
        )
    return OpenAI()
...
def call_openai_with_retries(
    ...
    """
    Call the OpenAI Responses API with retries and minimal length control
    ...
    """
    # Base payload (minus API key) – stored verbatim for reproducibility
    ...
    for attempt in range(1, gen_args.max_attempts + 1):
        ...
        try:
            ...
            logger.debug(
                "Calling OpenAI (attempt %d/%d, max_output_tokens=%d, min_required_tokens=%d)",
                attempt,
            ...
            response = client.responses.create(**payload)  # type: ignore[attr-defined]
            # Extract continuation text: response.output[0].content[0].text
            try:
                output0 = response.output[0]
                content0 = output0.content[0]
                continuation_text = content0.text  # type: ignore[attr-defined]
            ...
            if usage_raw is not None:
                if isinstance(usage_raw, dict):
                    ...
                    # Fallback: pull from __dict__ and drop callables/private attrs
            ...
            # SPECIAL CASE: empty string => accept immediately, no retries
            if continuation_text == "":
                logger.info(
                    "Model returned empty string; accepting as final output and skipping retries."
                )
                # We do not enforce min_required_tokens here; min_satisfied remains False.
                break
            ...
            # With temperature=0, retries will often give the same result; retries mainly help when
            # using stochastic decoding.
        except Exception as e:  # pragma: no cover - be robust to any API/library issues
            last_exc = e
            logger.warning("OpenAI call failed on attempt %d/%d: %s", attempt, gen_args.max_attempts, e)
...
def generate_single_completion(
    ...
    min_floor = 20.0 * r
    # NEW: use the smaller of the two as the enforced minimum
    ...
    # Construct full text with AI continuation – we keep the trimmed first half here.
    ...
    out_record: Dict[str, Any] = {
        ...
        "source": {
            ...
            "text": text,
            # we intentionally drop the original "parse" to keep files lighter
...
def process_language_file(
    ...
    Uses a thread pool to run multiple OpenAI requests concurrently.
    """
    ...
    processed_records = 0  # number of input records (items) actually scheduled
    ...
    with out_path.open("a", encoding="utf-8") as out_f:
        with ThreadPoolExecutor(max_workers=max(1, int(concurrency))) as ex:
            ...
            # Collect results as they complete; write and update stats in main thread
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Generate GPT-4.1 continuations for split-halves prompts in 35 languages."
    )
    p.add_argument(
        "--input-dir",
        type=Path,
    ...
    p.add_argument(
        ...
        default=1,
        help=(
            "Number of concurrent OpenAI requests per language (default: 1). "
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    # Check that we have prompt configs for all languages to be processed
    ...
    logger.info(
        "Starting GPT continuation generation run_id=%s for langs=%s with concurrency=%d, rpm=%d",
        run_id,
        ",".join(langs_to_run),
        args.concurrency,
        args.rpm,
    ...
    # Main per-language loop (sequential over languages; concurrency within each language)
